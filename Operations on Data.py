### Data Import and CleaningData Import and Cleaning
...
print(mean_values)
print("\nStandard deviation of reshaped array columns:")
print(std_values)

## 2.Stack two DataFrames vertically and use NumPy to analyze the combined dataset
import pandas as pd
import numpy as np
...
# Analyze the combined dataset using NumPy
mean_values = np.mean(combined_array, axis=0)
std_values = np.std(combined_array, axis=0)

print("\nMean values of combined dataset columns:")
print(mean_values)
print("\nStandard deviation of combined dataset columns:")
print(std_values)
...
# Convert the 3D array into a MultiIndex DataFrame
index = pd.MultiIndex.from_product([['Group1', 'Group2'], ['A', 'B', 'C'], [1, 2, 3, 4]], 
                                   names=['Group', 'Category', 'Item'])
df = pd.DataFrame(array_3d.reshape(-1, 1), index=index, columns=['Value'])
print("\nMultiIndex DataFrame:")
print(df)

# Perform a group operation (e.g., calculate the sum of 'Value' for each 'Group' and 'Category')
grouped_df = df.groupby(['Group', 'Category']).sum()
print("\nGrouped DataFrame with sum:")
print(grouped_df)

### Time Series Analysis
## 1.Convert a column in a DataFrame to a time series using Pandas and perform a NumPy-based operation on the time series data
import pandas as pd
import numpy as np

# Sample DataFrame with a date column and a value column
data = {
...
# Set the 'Date' column as the index to convert it into a time series
df.set_index('Date', inplace=True)
print("\nDataFrame with 'Date' as index (Time Series):")
print(df)

# Perform a NumPy-based operation (e.g., calculate the rolling mean with a window size of 3)
df['Rolling_Mean'] = df['Value'].rolling(window=3).mean()
print("\nDataFrame after calculating rolling mean:")
print(df)

## 2.Calculate the moving average of a time series in a DataFrame using a combination of Pandas and NumPy functions
import pandas as pd
import numpy as np
...
# Calculate the moving average with a window size of 3 using Pandas and NumPy
df['Moving_Average'] = df['Value'].rolling(window=3).mean()
print("\nDataFrame after calculating moving average:")
print(df)

# Optional: Using NumPy functions
moving_average_np = np.convolve(df['Value'], np.ones(3)/3, mode='valid')
print("\nMoving average using NumPy functions:")
print(moving_average_np)

## 3.Use pd.to_datetime to convert a string column to datetime format and use NumPy to compute the time difference between rows
import pandas as pd
import numpy as np

# Sample DataFrame with a string column representing dates
data = {
...
# Compute the time difference between rows using NumPy
df['Time_Diff'] = df['Date'].diff().dt.days
print("\nDataFrame after computing time difference between rows:")
print(df)
